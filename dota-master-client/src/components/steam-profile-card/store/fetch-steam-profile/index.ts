...
import SteamProfileModel from "../../models/steamProfile";
import profileApi from "@components/steam-profile-card/api/profile";
...
const fetchSteamProfileSlice = createSlice({
  ...
  initialState,
  reducers: {
    fetchSteamProfileRequest: (
      state
    ) => {
      state.loading = true;
    },
    fetchSteamProfileSuccess: (state,
...
function* fetchSteamProfile(): Generator<CallEffect<AxiosResponse> | PutEffect, void, ApiResponse> {
  try {
    const response: ApiResponse = yield call(profileApi.get);
    yield put(fetchSteamProfileSlice.actions.fetchSteamProfileSuccess(response.data));
  } catch (error) {
    yield put(
...
function* watchFetchSteamProfile() {
  yield takeLatest(fetchSteamProfileSlice.actions.fetchSteamProfileRequest.type, fetchSteamProfile);
}
