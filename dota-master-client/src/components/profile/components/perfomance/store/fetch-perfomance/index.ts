...
import profileApi from "@src/components/profile/components/perfomance/api/profile";
...
export const fetchHeroPerfomanceSliceReducer =
  fetchHeroPerfomanceSlice.reducer;
...
> {
  try {
    ...
    yield put(
      fetchHeroPerfomanceSlice.actions.fetchHeroPerfomanceSuccess(
        response.data
      )
    );
  } catch (error) {
