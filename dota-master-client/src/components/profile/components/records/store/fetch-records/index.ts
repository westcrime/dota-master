...
import profileApi from "@src/components/profile/components/records/api/profile";
...
const fetchRecordsSlice = createSlice({
  ...
  reducers: {
    ...
    fetchRecordsSuccess: (
      state,
      action: PayloadAction<RecordsModel>
    ) => {
      state.loading = false;
...
export const { fetchRecordsRequest, resetState } =
  fetchRecordsSlice.actions;
export const fetchRecordsSliceReducer =
  fetchRecordsSlice.reducer;
...
> {
  try {
    ...
    yield put(
      fetchRecordsSlice.actions.fetchRecordsSuccess(
        response.data
      )
    );
  } catch (error) {
