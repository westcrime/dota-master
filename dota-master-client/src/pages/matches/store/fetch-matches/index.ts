...
import profileApi from "@src/pages/matches/api/profile";
...
const fetchMatchBasicInfoSlice = createSlice({
  ...
  reducers: {
    fetchMatchBasicInfoRequest: (
      state,
      action: PayloadAction<{ limit?: number; offset?: number, heroId?: number }>
    ) => {
...
function* fetchMatchBasicInfo(
  ...
  try {
    ...
    const response: ApiResponse = yield call(profileApi.get, limit, offset, heroId);
    yield put(
